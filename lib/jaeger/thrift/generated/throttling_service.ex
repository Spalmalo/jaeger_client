defmodule(Jaeger.Thrift.Generated.ThrottlingService) do
  ...
  defmodule(GetThrottlingConfigsArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize__service_names(<<string_size::32-signed, element::binary-size(string_size), rest::binary>>, [list, remaining | stack])) do
      ...
      def(serialize(%GetThrottlingConfigsArgs{service_names: service_names})) do
        [case(service_names) do
          nil ->
            <<>>
          _ ->
            [<<15, 1::16-signed, 11, length(service_names)::32-signed>> | for(e <- service_names) do
  ...
  defmodule(GetThrottlingConfigsResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<12, 0::16-signed, rest::binary>>, acc)) do
        case(Elixir.Jaeger.Thrift.Generated.ThrottlingResponse.BinaryProtocol.deserialize(rest)) do
          {value, rest} ->
      ...
      def(serialize(%GetThrottlingConfigsResponse{success: success})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::16-signed>> | Jaeger.Thrift.Generated.ThrottlingResponse.serialize(success)]
        end | <<0>>]
      end
    end
    def(serialize(struct)) do
  ...
  defmodule(Binary.Framed.Client) do
    ...
    def(unquote(:get_throttling_configs)(client, service_names, rpc_opts \\ [])) do
      ...
      ClientImpl.call(client, "getThrottlingConfigs", serialized_args, GetThrottlingConfigsResponse.BinaryProtocol, rpc_opts)
    end
    def(unquote(:get_throttling_configs!)(client, service_names, rpc_opts \\ [])) do
  ...
  defmodule(Binary.Framed.Server) do
    ...
    def(handle_thrift("getThrottlingConfigs", binary_data, handler_module)) do
      case(Elixir.Jaeger.Thrift.Generated.ThrottlingService.GetThrottlingConfigsArgs.BinaryProtocol.deserialize(binary_data)) do
        {%Jaeger.Thrift.Generated.ThrottlingService.GetThrottlingConfigsArgs{service_names: service_names}, ""} ->
          try do
            rsp = handler_module.get_throttling_configs(service_names)
            (
              response = %Jaeger.Thrift.Generated.ThrottlingService.GetThrottlingConfigsResponse{success: rsp}
              {:reply, Elixir.Jaeger.Thrift.Generated.ThrottlingService.GetThrottlingConfigsResponse.BinaryProtocol.serialize(response)}
            )
          catch
            kind, reason ->
