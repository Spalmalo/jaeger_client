defmodule(Jaeger.Thrift.Generated.ZipkinCollector) do
  @moduledoc false
      ...
      def(serialize(%SubmitZipkinBatchArgs{spans: spans})) do
        [
          case(spans) do
            ...
            _ ->
              [
                <<15, 1::16-signed, 12, length(spans)::32-signed>>
                | for(e <- spans) do
  ...
  defmodule(SubmitZipkinBatchResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize__success(<<rest::binary>>, [list, remaining | stack])) do
        case(Elixir.Jaeger.Thrift.Generated.Response.BinaryProtocol.deserialize(rest)) do
          {element, rest} ->
          ...
          case(success) do
            ...
            _ ->
              [
                ...
                | for(e <- success) do
                    Jaeger.Thrift.Generated.Response.serialize(e)
                  end
              ]
          end
          | <<0>>
  ...
  defmodule(Binary.Framed.Client) do
    ...
    def(unquote(:submit_zipkin_batch)(client, spans, rpc_opts \\ [])) do
      ...
      ClientImpl.call(
        ...
        "submitZipkinBatch",
        serialized_args,
        SubmitZipkinBatchResponse.BinaryProtocol,
        rpc_opts
      )
    end
  ...
  defmodule(Binary.Framed.Server) do
    ...
    def(handle_thrift("submitZipkinBatch", binary_data, handler_module)) do
      case(
        Elixir.Jaeger.Thrift.Generated.ZipkinCollector.SubmitZipkinBatchArgs.BinaryProtocol.deserialize(
          binary_data
        )
      ) do
        {%Jaeger.Thrift.Generated.ZipkinCollector.SubmitZipkinBatchArgs{spans: spans}, ""} ->
          try do
            rsp = handler_module.submit_zipkin_batch(spans)

            (
              response = %Jaeger.Thrift.Generated.ZipkinCollector.SubmitZipkinBatchResponse{
                success: rsp
              }

              {:reply,
               Elixir.Jaeger.Thrift.Generated.ZipkinCollector.SubmitZipkinBatchResponse.BinaryProtocol.serialize(
                 response
               )}
            )
          rescue
