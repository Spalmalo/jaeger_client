defmodule(Jaeger.Thrift.Generated.Collector) do
  @moduledoc false
      ...
      def(serialize(%SubmitBatchesArgs{batches: batches})) do
        [
          case(batches) do
            ...
            _ ->
              [
                <<15, 1::16-signed, 12, length(batches)::32-signed>>
                | for(e <- batches) do
                    Jaeger.Thrift.Generated.Batch.serialize(e)
                  end
              ]
          end
          | <<0>>
  ...
  defmodule(SubmitBatchesResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize__success(<<rest::binary>>, [list, remaining | stack])) do
        case(
          Elixir.Jaeger.Thrift.Generated.BatchSubmitResponse.BinaryProtocol.deserialize(rest)
        ) do
          {element, rest} ->
          ...
          case(success) do
            ...
            _ ->
              [
                ...
                | for(e <- success) do
                    Jaeger.Thrift.Generated.BatchSubmitResponse.serialize(e)
                  end
              ]
          end
          | <<0>>
  ...
  defmodule(Binary.Framed.Client) do
    ...
    def(unquote(:submit_batches)(client, batches, rpc_opts \\ [])) do
      ...
      ClientImpl.call(
        ...
        "submitBatches",
        serialized_args,
        SubmitBatchesResponse.BinaryProtocol,
        rpc_opts
      )
    end
  ...
  defmodule(Binary.Framed.Server) do
    ...
    def(handle_thrift("submitBatches", binary_data, handler_module)) do
      case(
        Elixir.Jaeger.Thrift.Generated.Collector.SubmitBatchesArgs.BinaryProtocol.deserialize(
          binary_data
        )
      ) do
        {%Jaeger.Thrift.Generated.Collector.SubmitBatchesArgs{batches: batches}, ""} ->
          try do
            rsp = handler_module.submit_batches(batches)

            (
              response = %Jaeger.Thrift.Generated.Collector.SubmitBatchesResponse{success: rsp}

              {:reply,
               Elixir.Jaeger.Thrift.Generated.Collector.SubmitBatchesResponse.BinaryProtocol.serialize(
                 response
               )}
            )
          catch
